library(rms)
library(MASS)
AllModelAllBinary<-function(form,FileRead,FileOut,bnum){
...
boot.optimateUnvali<-function(form,mydata,file,filezong,bootnum,app.z){
  mynewdata<-cal.binaryAll(mydata,form)
  auc.boot<-0.0
  R2.boot<-0.0
  sloop.boot<-0.0
  auc.test<-0.0
  ...
  for(i in 1:bootnum){
    ...
    set.seed(13*a[i])
    bootdata<-boot.sample(mydata)
    bootdata<-cal.binaryAll(bootdata,form)
    ...
    if(nlen==0){
      ...
      fitnew<-lrm(form.v1,x=TRUE, y=TRUE, data = bootdata)
      boot.par<-calcum.boot(fitnew)#boot的三个指标
      test.par<-calcum.test(fitnew,mynewdata,form.v1)
      opti.par<-boot.par-test.par
      z.boot<-z.boot+boot.par
      z.test<-z.test+test.par
    }
...
boot.LRUnvali<-function(form,fileread,fileframe,filemodel,filezong,bnum){
  myolddata<-read.table(file=fileread,header=TRUE,sep=",")
  mydata<-cal.binaryAll(myolddata,form)#对原值数据进行二分类
  app.z<-calcum.apparent(form,mydata,filemodel)#计算出原始值
  nlen<-length(app.z)
  form.v1<-pack.form(app.z[4:nlen])
  len<-length(app.z)
  auc<-as.numeric(app.z[1])
  ...
  slope<-as.numeric(app.z[3])#需要转化为数值型
  app.z<-c(auc,r2,slope)#原始数据
  z<-boot.optimateUnvali(form,myolddata,filemodel,filezong,bnum,app.z)
  opt.z<-z[7:9]
  boot.z<-z[1:3]
  test.z<-z[4:6]
  true.z<-app.z-opt.z
...
cal.statDiff<-function(mydata,data){
 ...
 hayida.bootstrap(form,FileRead,FileOut,bnum)
}
#如何对数据进行二分类
form=Y~X1+X2+X3+X4+X7+X10+X11+X12+X13+X14+X15
#mydata<-read.table(file='原始数据.csv',header=TRUE,sep=",")
#bootdata<-boot.sample(mydata)
AllModelAllBinary(form,'原始数据.csv','data/v31',bnum=10)
