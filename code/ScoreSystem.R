...
library(MASS)
##全模型部分自动二分类数据
ScoreSystem<-function(form,fileRead,fileOut,bnum,modelType){
  fileThraval<-paste0(fileOut,'/thraval.csv')
...
#bootstrap过程，无单因素分析
boot.optimateUnvali<-function(form,mydata,file,filezong,bootnum,app.z,model.n){
  auc.boot<-0.0
  R2.boot<-0.0
  sloop.boot<-0.0
  auc.test<-0.0
  ...
  for(i in 1:bootnum){
    ...
    set.seed(13*a[i])
    bootdata<-boot.sample(mydata)
#    filetemp1<-paste('boot/Mixdata',i,'.csv',sep='')
#    write.table(bootdata,file=filetemp1,sep=',',dec='.',col.names=TRUE,row.names=FALSE)
    bootdata<-cal.data(bootdata,model.n)
#    filetemp2<-paste('boot/MixScoreGrade',i,'.csv',sep='')
#    write.table(bootdata,file=filetemp2,sep=',',dec='.',col.names=TRUE,row.names=FALSE)
    ...
    if(nlen==0){
      ...
      fitnew<-lrm(form.v1,x=TRUE, y=TRUE, data = bootdata)
      boot.par<-calcum.boot(fitnew)#boot的三个指标
      if(model.n==2||model.n==4){
        thre1<-bootdata[100,'cut']
        thre2<-bootdata[200,'cut']
        thre3<-bootdata[260,'cut']
        three.value<-c(thre1,thre2,thre3)
        if(model.n==2){
          mydatanew<-cal.MixDataScore(mydata)
        }
        else{
          mydatanew<-cal.OrigDataScore(mydata)
        }
        data<-modelAUC.ThreeData(mydatanew,three.value)#根据阀值转换
        thredValue<-data.frame(thre1,thre2,thre3)
        write.table(thredValue,fileThraval,append=TRUE,col.names = FALSE,row.names = FALSE,sep = ",",eol = "\n")
      }
      else{
        data<-cal.data(mydata,model.n)
      }
      test.par<-calcum.test(fitnew,data,form.v1)#此处有修改
      opti.par<-boot.par-test.par
      z.boot<-z.boot+boot.par
      z.test<-z.test+test.par
    }
...
############################
#无单因素分析
boot.LRUnvali<-function(form,fileread,fileframe,filemodel,filezong,bnum,model.n){
  mydataOrig<-read.table(file=fileread,header=TRUE,sep=",")
  mydata<-cal.data(mydataOrig,model.n)
  form.score<-Y~Score
  app.z<-calcum.apparent(form.score,mydata,filemodel)#计算出原始值  使用全模型
  nlen<-length(app.z)
  form.v1<-pack.form(app.z[4:nlen])
  len<-length(app.z)
  auc<-as.numeric(app.z[1])
  ...
  slope<-as.numeric(app.z[3])#需要转化为数值型
  app.z<-c(auc,r2,slope)#原始数据
  z<-boot.optimateUnvali(form,mydataOrig,filemodel,filezong,bnum,app.z,model.n)
  opt.z<-z[7:9]
  boot.z<-z[1:3]
  test.z<-z[4:6]
  true.z<-app.z-opt.z
...
hayida<-function(form,fileread,fileframe,filemodel,filezong,bnum,model.n){
  boot.LRUnvali(form,fileread,fileframe,filemodel,filezong,bnum,model.n)
}
...
#定义函数接口
hayida.bootstrap<-function(form,fileread,fileout='data',bnum,model.n){
  ...
  if(!file.exists(fileread)){
    ...
    hayida(form,fileread,fileframe,filemodel,filezong,bnum,model.n)
...
#计算混合数据的得分
cal.MixDataScore<-function(mydata){
  mydata$Score=70*mydata$X4+111*mydata$X3+80*mydata$X13+75*mydata$X12+
    60*mydata$X7+2*mydata$X2+2*mydata$X11+1*mydata$X14
  mydata
}
#计算原始数据的得分
cal.OrigDataScore<-function(mydata){
  mydata$Score=70*mydata$X4+135*mydata$X3-50*mydata$X13+0.15*mydata$X12+
    1*mydata$X7+2.5*mydata$X2+2*mydata$X11+1*mydata$X14
  mydata
}
#划分混合数据级别方法1
modelAUC.MixDataScore<-function(mydata){
  len=nrow(mydata)
  for(i in 1:len){
    if(mydata[i,'Score']<=382)
      mydata[i,'Score']<-1
    else if(mydata[i,'Score']>382&&mydata[i,'Score']<=500)
      mydata[i,'Score']<-2
    else
      mydata[i,'Score']<-3
  }
...
#根据三个阀值点进行划分
modelAUC.ThreeData<-function(mydata,zd){
  len=nrow(mydata)
  for(i in 1:len){
    if(mydata[i,'Score']<=zd[1])
      mydata[i,'Score']<-1
    else if(mydata[i,'Score']>zd[1]&&mydata[i,'Score']<=zd[2])
      mydata[i,'Score']<-2
    else
      mydata[i,'Score']<-3
  }
...
#划分原始数据级别方法1
modelAUC.OrigDataScore<-function(mydata){
  len=nrow(mydata)
  for(i in 1:len){
    if(mydata[i,'Score']<=286)
      mydata[i,'Score']<-1
    else if(mydata[i,'Score']>286&&mydata[i,'Score']<=390)
      mydata[i,'Score']<-2
    else
      mydata[i,'Score']<-3
  }
...
#按照比例对数据进行划分
modelPropor.data<-function(mydata){
  mydata<-mydata[order(mydata$Score,decreasing=F),]#Score从小到大进行排序
  data.1<-mydata[105,'Score']
  data.2<-mydata[210,'Score']
  data.3<-mydata[264,'Score']
  for(i in 1:105){
    mydata[i,'Score']<-1
    mydata[i,'cut']<-data.1
  }
...
#处理数据
cal.data<-function(mydata,model.num){
  #处理混合数据的第一种方法
  if(model.num==1){
    mydata<-cal.MixDataScore(mydata)
    mydata<-modelAUC.MixDataScore(mydata)
  }
  #处理混合数据的第二种方法
  else if(model.num==2){
    mydata<-cal.MixDataScore(mydata)
    mydata<-modelPropor.data(mydata)
  }
  #处理原始数据的第一种方法
  else if(model.num==3){
    mydata<-cal.OrigDataScore(mydata)
    mydata<-modelAUC.OrigDataScore(mydata)
  }
  #处理原始数据的第二种方法
  else if(model.num==4){
    mydata<-cal.OrigDataScore(mydata)
 ...
 hayida.bootstrap(form,fileRead,fileOut,bnum,modelType)
}

ScoreSystem(form,'原始数据.csv','data/model2/v14',bnum=20,4)
